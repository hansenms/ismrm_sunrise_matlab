...
unmix_sense2 = ismrm_calculate_sense_unmixing(acc_factor, csm_prew);
im_hat(:,:,2) = sum(im_alias_prew .* unmix_sense2, 3);

ccm_prew = ismrm_compute_ccm(csm_prew);
gmap(:,:,2) = ismrm_calculate_gmap(unmix_sense2, ccm_prew);
...
% reconstruction.
unmix_sense3 = ismrm_calculate_sense_unmixing(acc_factor, csm, Rn);
