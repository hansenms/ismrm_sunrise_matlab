...
if 0 % todo: old
	...
	if numel(lo) > 2
		pad = (numel(lo) - 2) / 2;
		z = zeros(pad, M);
		coef_lolo = [z; coef_lolo; z]; % [N/2+2*pad M]
		coef_lohi = [z; coef_lohi; z];
		coef_hilo = [z; coef_hilo; z];
		coef_hihi = [z; coef_hihi; z];
	else
		pad = 0;
	end

	coef_lo = conv2(coef_lolo, lo', 'same') + conv2(coef_lohi, hi', 'same');
	coef_hi = conv2(coef_hilo, lo', 'same') + conv2(coef_hihi, hi', 'same');

	id = 2:2:N;
	tmp = zeros(N, M, class(coef));
	up_lo = tmp;
	up_hi = tmp;
	up_lo(id,:) = coef_lo; % [N/2+2*pad M]
	up_hi(id,:) = coef_hi;

	if numel(lo) > 2
		warn 'todo'
		z = zeros(N/2+2*pad, pad);
	size(z)
		coef_lo = [z, coef_lo, z];
	size(coef_hi)
		coef_hi = [z, coef_hi, z];
	...
	xx = conv2(up_lo, lo, 'same') + conv2(up_hi, hi, 'same');
	equivs(x, xx)
end
