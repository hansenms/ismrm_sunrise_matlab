...
%|
%| ADMM based model:
%|	1/2 ||h - A u||^2 + 1/2 ||u - Ts - eta||_V^2
%|	so u = [u1 u0]', T = [I' C']', h = [sqrt(W)z 0]', A = [sqrt(W)D
%0 ; 0 sqrt(lam)B], eta = [eta1 eta0], and V = [v1I 0; 0 v0I]
%|
%| in
%|	ykj	[nx ny ncoil]	noisy complex images for each coil
%|
%| options
%|	bodycoil	[nx ny]	coil image
...
%% create regularizer
[C,B,phi] = ir_circ_zeroed_reg(arg.order, true([nx ny]), 0);
...
disp(['Actual v0:' num2str(arg.v0) ' v1:' num2str(arg.v1)]);
disp(['Actual K(u0):' num2str(condu0Act) ' K(s):' num2str(condsAct) ...
...
for i = 1:ncoil % it is better to do this in parallel as they are independent.
	...
	% initialize smap
	sk = ir_mri_sensemap_init(arg.init, yj, arg.bodycoil, [nx ny 1], ...
			arg.thresh, []);
	sk = sk(:);
	sinit(:,i) = sk;
	...
	for j = 1:arg.niter
		...
		% update sk
		sk = ifft2(GinvS .* fft2(reshape(C' * (u0 - eta0) + v1ov0 * ...
			(u1 - eta1), [nx ny])));
		sk = sk(:);
		...
		%	disp('outputting eta1');
			disp([num2str(j) ' of ' num2str(arg.niter)])
		end
	end % for j
end % for i
...
disp('Creating sensitivity data.')
...
if 1
	disp('Starting Cholesky based method')
	disp('-- Computing C');
