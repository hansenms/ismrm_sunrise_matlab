/*******************************************************************************
        ...
        gradient interpolation of transpose using zero boundary condition.
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	{
		mexPrintf("VAL = BsplCo2GdXTranZero(COEFF, [nx ny (nz)], [offx"
	...
	{
                mexErrMsgTxt("Fifth argument should be either {}, {wx wy wz}"
			" or {wx wy}\n");
		return;
	}
	...
	/* Retrieve the output data */
        ...
        if (ntot == 0) /* no warp */
        {
                if (dim == 3)
                {
                        BatchInterpolatedGradX3DTranZero(data2, data1, m, n, l,
 				nx, offx, mx, ny, offy, my, nz, offz, mz, 
				(long)spline);
                }
                else
                {
                        BatchInterpolatedGradX2DTranZero(data2, data1, m, n, 
        ...
        else
        {
                if (dim == 3)
                {
                        BatchInterpolatedGradX3DTranZeroWarp(data2, data1, m, n,
 				l, nx, offx, mx, wx, ny, offy, my, wy, nz, offz,
 				mz, wz, (long)spline);
                }
                else
                {
                        BatchInterpolatedGradX2DTranZeroWarp(data2, data1, m, n,
