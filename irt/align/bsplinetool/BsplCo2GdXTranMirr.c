/*******************************************************************************
        ...
        gradient interpolation of transpose using mirror condition.
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	{
		mexPrintf("VAL = BsplCo2GdXTranMirr(COEFF, [nx ny (nz)], [offx"
	...
	/* Retrieve the output data */
        ...
        if (ntot == 0) /* no warp */
        {
                if (dim == 3)
                {
                        BatchInterpolatedGradX3DTranMirr(data2, data1, m, n, l,
 				nx, offx, mx, ny, offy, my, nz, offz, mz, 
				(long)spline);
                }
                else
                {
                        BatchInterpolatedGradX2DTranMirr(data2, data1, m, n, 
        ...
        else
        {
                if (dim == 3)
                {
                        BatchInterpolatedGradX3DTranMirrWarp(data2, data1, m, n,
 				l, nx, offx, mx, wx, ny, offy, my, wy, nz, offz,
 				mz, wz, (long)spline);
                }
                else
                {
                        BatchInterpolatedGradX2DTranMirrWarp(data2, data1, m, n,
