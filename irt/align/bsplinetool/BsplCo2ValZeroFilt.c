/*******************************************************************************
        ...
        interpolation using filtering technique with zero end condition.
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	{
                mexPrintf("VAL = BsplCo2ValZeroFilt(COEFF, [nx ny (nz)], [offx"
		 " offy (offz)], [mx my (mz)], {KERNELx KERNELy (KERNELz)});\n");
                mexPrintf("[in]\n");
			...
			" direction\n");
                mexPrintf("\tKERNEL             : FIR kernel filter"
			" coefficients, bm^n\n");
                mexPrintf("[out]\n");
	...
	if (ntot == 2)
	{
		...
		ny = (int)*((double*)(mxGetData(prhs[1]) + sizebuf));
		nz = 1;
		dims[0] = nx;
		dims[1] = ny;
	}
	else if (ntot == 3)
	{
		nx = (int)*((double*)mxGetData(prhs[1]));
		ny = (int)*((double*)(mxGetData(prhs[1]) + sizebuf));
		nz = (int)*((double*)(mxGetData(prhs[1]) + 2*sizebuf));
		dims[0] = nx;
		dims[1] = ny;
		dims[2] = nz;
	}
	...
	if (ntot == 2)
	{
		...
		offy = (int)*((double*)(mxGetData(prhs[2]) + sizebuf));
		offz = 0;
	}
	else if (ntot == 3)
	{
		offx = (int)*((double*)mxGetData(prhs[2]));
		offy = (int)*((double*)(mxGetData(prhs[2]) + sizebuf));
		offz = (int)*((double*)(mxGetData(prhs[2]) + 2*sizebuf));
	}
	else
	{
	...
	if (ntot == 2)
	{
		...
		my = (int)*((double*)(mxGetData(prhs[3]) + sizebuf));
		mz = 0;
	}
	else if (ntot == 3)
	{
		mx = (int)*((double*)mxGetData(prhs[3]));
		my = (int)*((double*)(mxGetData(prhs[3]) + sizebuf));
		mz = (int)*((double*)(mxGetData(prhs[3]) + 2*sizebuf));
	}
	else
	{
	...
	if (ntot == 3)
	{
		FiltInterpolatedValue3DZero(data2, data1, m, n, l, nx, offx, mx,
 			ny, offy, my, nz, offz, mz, 
			kernelx, nkx, kernely, nky, kernelz, nkz); 
	}
	else
	{
		FiltInterpolatedValue2DZero(data2, data1, m, n, nx, offx, mx, ny,
 			offy, my, kernelx, nkx, kernely, nky); 
	}
}
