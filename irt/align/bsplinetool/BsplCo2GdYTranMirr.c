...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	{
		mexPrintf("VAL = BsplCo2GdYTranMirr(COEFF, [nx ny (nz)], [offx"
	...
	/* Retrieve the output data */
        ...
        if (ntot == 0) /* no warp */
        {
                if (dim == 3)
                {
                        BatchInterpolatedGradY3DTranMirr(data2, data1, m, n, l,
 				nx, offx, mx, ny, offy, my, nz, offz, mz, 
				(long)spline);
                }
                else
                {
                        BatchInterpolatedGradY2DTranMirr(data2, data1, m, n, 
        ...
        else
        {
                if (dim == 3)
                {
                        BatchInterpolatedGradY3DTranMirrWarp(data2, data1, m, n,
 				l, nx, offx, mx, wx, ny, offy, my, wy, nz, offz,
 				mz, wz, (long)spline);
                }
                else
                {
                        BatchInterpolatedGradY2DTranMirrWarp(data2, data1, m, n,
