/*******************************************************************************
        ...
        gradient interpolation using zero end condition.
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	{
                mexPrintf("VAL = BsplCo2GdXZero(COEFF, [nx ny (nz)], [offx "
			...
			"{default: 3}\n");
                mexPrintf("[out]\n");
                mexPrintf("\tVAL                : interpolated grad x "
			"values\n");
		return;
	}
	...
	{
                mexErrMsgTxt("Fifth argument should be {}, {wx wy wz}"
			" or {wx wy}\n");
		return;
	}
	...
	if (ntot == 0)
	{
		if (dim == 3)
		{
			BatchInterpolatedGradX3DZero(data2, data1, m, n, l,
 				nx, offx, mx, ny, offy, my, nz, offz, mz, 
				(long)spline); 
		}
		else
		{
			BatchInterpolatedGradX2DZero(data2, data1, m, n, 
	...
	else
	{
		if (dim == 3)
		{
			BatchInterpolatedGradX3DZeroWarp(data2, data1, m, 
				n, l, nx, offx, mx, wx, ny, offy, my, wy, 
				nz, offz, mz, wz, (long)spline); 
		}
		else
		{
			BatchInterpolatedGradX2DZeroWarp(data2, data1, m, n, nx,
