...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	{
		mexPrintf("VAL = BsplCo2GdZTranMirr(COEFF, [nx ny (nz)], [offx"
	...
	/* Retrieve the output data */
        ...
        if (ntot == 0) /* no warp */
        {
                BatchInterpolatedGradZ3DTranMirr(data2, data1, m, n, l, nx, offx
			, mx, ny, offy, my, nz, offz, mz, (long)spline);
        }
        else
        {
                BatchInterpolatedGradZ3DTranMirrWarp(data2, data1, m, n, l, nx,
 			offx, mx, wx, ny, offy, my, wy, nz, offz, mz, wz, 
			(long)spline);
        }
}
