...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	{
                mexPrintf("VAL = BsplCo2GdXMirr(COEFF, [nx ny (nz)], [offx "
	...
	if (ntot == 0)
	{
		if (dim == 3)
		{
			BatchInterpolatedGradX3DMirr(data2, data1, m, n, l,
 				nx, offx, mx, ny, offy, my, nz, offz, mz, 
				(long)spline); 
		}
		else
		{
			BatchInterpolatedGradX2DMirr(data2, data1, m, n, 
	...
	else
	{
		if (dim == 3)
		{
			BatchInterpolatedGradX3DMirrWarp(data2, data1, m, 
				n, l, nx, offx, mx, wx, ny, offy, my, wy, 
				nz, offz, mz, wz, (long)spline); 
		}
		else
		{
			BatchInterpolatedGradX2DMirrWarp(data2, data1, m, n, nx,
