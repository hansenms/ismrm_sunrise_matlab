/*******************************************************************************
        ...
        interpolation of transpose using zero end condition.
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	{
		mexPrintf("VAL = BsplCo2ValTranZero(COEFF, [nx ny (nz)], [offx"
	...
	/* Retrieve the output data */
        ...
        if (ntot == 0) /* no warp */
        {
                if (dim == 3)
                {
                        BatchInterpolatedValue3DTranZero(data2, data1, m, n, l,
 				nx, offx, mx, ny, offy, my, nz, offz, mz, 
				(long)spline);
                }
                else
                {
                        BatchInterpolatedValue2DTranZero(data2, data1, m, n, nx,
        ...
        else
        {
                if (dim == 3)
                {
                        BatchInterpolatedValue3DTranZeroWarp(data2, data1, m, n,
 				l, nx, offx, mx, wx, ny, offy, my, wy, nz, 
				offz, mz, wz, (long)spline);
                }
                else
                {
                        BatchInterpolatedValue2DTranZeroWarp(data2, data1, m, n,
