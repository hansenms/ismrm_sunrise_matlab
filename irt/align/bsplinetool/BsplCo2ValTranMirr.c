/*******************************************************************************
        ...
        interpolation of transpose using mirror condition.
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	{
		mexPrintf("VAL = BsplCo2ValTranMirr(COEFF, [nx ny (nz)], [offx"
	...
	/* Retrieve the output data */
        ...
        if (ntot == 0) /* no warp */
        {
                if (dim == 3)
                {
                        BatchInterpolatedValue3DTranMirr(data2, data1, m, n, l,
 				nx, offx, mx, ny, offy, my, nz, offz, mz, 
				(long)spline);
                }
                else
                {
                        BatchInterpolatedValue2DTranMirr(data2, data1, m, n, nx,
        ...
        else
        {
                if (dim == 3)
                {
                        BatchInterpolatedValue3DTranMirrWarp(data2, data1, m, n,
 				l, nx, offx, mx, wx, ny, offy, my, wy, nz, 
				offz, mz, wz, (long)spline);
                }
                else
                {
                        BatchInterpolatedValue2DTranMirrWarp(data2, data1, m, n,
