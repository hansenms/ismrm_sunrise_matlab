...
if isfield(st, 'phase_after') && ~isempty(st.phase_after) % adjoint via conjugate
...
if isfield(st, 'interp_adj') % table or other functional method
	...
	if length(om) ~= dims(1), error 'om vs X size mismatch', end
	Xk_all = st.interp_adj(X, om);
else
	if isvar('om') && ~isempty(om) && ~isequal(om, st.om)
		error 'om given that does not match st.om'
	end
	if length(st.om) ~= dims(1), error 'om vs X size mismatch', end
