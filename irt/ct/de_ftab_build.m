...
if ~isvar('ftab.inv_water'), printm 'water inv'
	...
	ftab.inv_water_eval = inline(ftab.inv_water_eval, 'ftab', 'm', 'f');
...
%	savefig('c', fdir, 'fig_f_linearized')
...
if ~isvar('ftab.inv.eval'), printm 'inv: f -> s'
	if 0	% this polynomial way seems insufficiently accurate!?
		ftab.inv.basis_str = poly_string(3);
		ftab.inv.basis_func = inline(ftab.inv.basis_str, 'x', 'y');
