...
%|
%| Solved by min_{D, S >=0} || D || s.t. H = S + D
%|
%| in
...
frob = sum(xs.^2, 1);
frob = frob / norm(x0).^2; % normalize to [0,1]
...
plot(0:niter, frob, '.-')
xlabel 'iteration'
title 'relative Frobenius norm'
axis([0 niter 0.98 1.01])
ytick([0.98 1 1.01])

im subplot 2
plot(0:niter, min(lam), '.-')
title 'minimum eigenvalue of D-H'
...
plot(0:niter, frobe, '.-')
title 'relative Frobenius norm of D+\alpha I'
ir_text(.5*niter, 1.002, '\alpha = -min(\lambda_{min}(D-H),0)', ...
...
legend('initial', 'final', 'location', 'south')
title 'diagonal values'
xlabel 'j'
ylabel 'D_{jj}'
axis([1 n 1.9 4.1])
ytick([2 3 4])
