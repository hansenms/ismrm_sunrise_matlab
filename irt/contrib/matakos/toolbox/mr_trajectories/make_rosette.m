function [kspace,omega,grad,slew,ts,idx,wi,rlen] = ...
	make_rosette(fov, N, dt, varargin)
...
%| Copyright 2010-10-28, Antonis Matakos, The University of Michigan
...
if ~isvar('fov')
	fov = 24;
elseif length(fov) > 1
	warning('make_rosette:FOVerror','FOV square, keeping only first value');
	fov = fov(1);
end
...
if ~isvar('N')
	N = 64;
elseif length(N) > 1
	warning('make_rosette:NSizeError','square matrix, keeping only first value');
	N = N(1);
end
...
[ks, om, gr, sl, ts, idx] = make_rosette(fov,N,dt,'Nt',Nt,'chat',0,'shot',nshot);
% [ks2, om2, gr2, sl2, ts2, idx2] = make_epi(fov,N,etl,dt,...
% 	'offset',1,'type','int','smpr',0,'dir',1);
