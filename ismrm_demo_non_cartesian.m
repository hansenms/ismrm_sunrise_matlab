...
ismrm_imshow([abs(recon_undersampled) abs(img_sense) abs(img_spirit)]); colormap(gray);
if (pseudo_replicas > 1),
    ismrm_imshow([abs(snr_sense) abs(snr_spirit)]); colormap(gray);
    ismrm_imshow([abs(g_sense) abs(g_spirit)]); colormap(gray);    
end
