...
im_alias = ismrm_transform_kspace_to_image(data_accel .* repmat(sp == 1 | sp == 3,[1 1 size(smaps,3)]),[1,2]);
...
cal_im = ismrm_transform_kspace_to_image(cal_data, [1,2]);
...
ismrm_imshow(m_full - m_grappa);
ismrm_imshow(m_full - m_pars);
ismrm_imshow(m_full - m_sense);
